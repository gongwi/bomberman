...
class Frames:  
    ...
    """frame = 帧/图片"""
    def __init__(self, images, on_timeout=None, interval=500, repeat=1,
                 horizontal_flip=False, vertical_flip=False, non_stop=True,
                 width=None, height=None):
        """
        images can be file paths or QImage objects
        """
        self.frames = []
        self.max_width = 0  
        self.max_height = 0  
...
class Animations:  
    ...
    def extract_images(self, image, n_rows, n_columns):  
        """
        extract individual sprites from a combined image
        """
    ...
    def add(self, name, images, interval=500, repeat=1, horizontal_flip=False,
            vertical_flip=False, non_stop=True, height=None, width=None):
        # images can be file paths or QImage objects
        frames = Frames(images, on_timeout=self.update_frame, interval=interval, 
                        repeat=repeat, horizontal_flip=horizontal_flip, 
                        vertical_flip=vertical_flip, non_stop=non_stop,
...
class MessageItem(QGraphicsItem):  
    ...
    def __init__(self, scene, x=0, y=240):
        super().__init__()
        self.scene = scene
        self.setX(x)
        self.setY(y)
        ...
        # self.Background,QtGui.QColor(255,255,255)
        self.timer = QtCore.QTimer()
